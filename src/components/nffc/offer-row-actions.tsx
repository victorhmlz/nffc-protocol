...
// No deployed Marketplace address yet (TASK-31) — same honest,
...
function simulateCancelOfferFixture(): Promise<void> {
  return Promise.reject(new Error("Marketplace is not deployed yet (TASK-31) — cancelling an offer is unavailable."));
}
function buildCancelOfferCallFixture(): WriteContractParams {
  throw new Error("unreachable — simulateCancelOfferFixture always rejects until Marketplace is deployed (TASK-31)");
}
function simulateAcceptOfferFixture(): Promise<void> {
  return Promise.reject(new Error("Marketplace is not deployed yet (TASK-31) — accepting an offer is unavailable."));
}
function buildAcceptOfferCallFixture(): WriteContractParams {
  throw new Error("unreachable — simulateAcceptOfferFixture always rejects until Marketplace is deployed (TASK-31)");
}
...
export function OfferRowActions({ offerId, buyerAddress, ownerAddress }: OfferRowActionsProps) {
  ...
  const cancelFlow = useMarketplaceActionFlow({
    simulate: simulateCancelOfferFixture,
    buildCall: buildCancelOfferCallFixture,
  });
  const acceptFlow = useMarketplaceActionFlow({
