...
| `MintConditionCard` / `OwnershipCard` / `ListingCard` / `OffersList` / `ActivityTimeline` (`src/components/nffc/`) | The `/nffc/[tokenId]` detail page's remaining panels — the frozen mint-condition trait, owner/creator, the current listing (reuses `BuyButton`), read-only active offers, and a traceable activity timeline (every entry carries a block number + tx hash) (TASK-21, `docs/nffc-detail.md`). |
